...
export default class GIF {
  ...
  private static COLORS: Uint8Array = (() => {
    const colors = new Uint8Array(3 * Math.pow(2, GIF.DEEP));
    let i = 0;
    // TRANSPARENT
    colors[i++] = 0xff;
    colors[i++] = 0x00;
    ...
    for (const key in COLORS) {
      const rgb = Color.RGB((<any>COLORS)[key]);
      ...
      if (hsv[1] >= 9) {
        for (let d = 0; d < 2; d++) {
          const dhsv = [hsv[0], (hsv[1] / 3) * (d + 1), hsv[2]];
          const drgb = Color.HSV2RGB(dhsv);
          colors[i++] = drgb[0];
          colors[i++] = drgb[1];
          colors[i++] = drgb[2];
  ...
  getColor(r: number, g: number, b: number) {
    ...
    for (let i = 0; i < GIF.COLORN; index++) {
      const cr = GIF.COLORS[i++];
      const cg = GIF.COLORS[i++];
      const cb = GIF.COLORS[i++];
  ...
  writePixels() {
    ...
    const enc = new LZW(width, height, GIF.DEEP);
